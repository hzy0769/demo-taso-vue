...
function submit() {
  ...
  setTimeout(() => {
    ...
    err.value = `Incorrect code. ${3 - fails.value} attempt${3 - fails.value > 1 ? 's' : ''} left.`
    shake()
...
<template>
  <section class="scr" :class="{ on: app.screen === 'auth-otp' }" data-screen="auth-otp">
    ...
    <h1 style="font-size:24px;font-weight:700;letter-spacing:-.02em;margin-top:8px">Enter verification code</h1>
    <p class="meta" style="margin-top:6px;font-size:12.5px">
      We sent a 6-digit code to <b style="color:var(--fg)">{{ app.auth.otpTo }}</b>
    </p>
    <div style="margin-top:10px"><span class="badge soft">演示验证码 123456</span></div>
    ...
    <div class="row" style="margin-top:14px;justify-content:center;min-height:20px">
      ...
      <p v-else class="meta" style="font-size:12px">{{ digitsLeft }} digits left</p>
    </div>
    ...
    <div style="text-align:center;margin-top:16px">
      <button
        ...
        :disabled="sec > 0"
        @click="resend"
      >{{ sec > 0 ? `Resend code in ${sec}s` : 'Resend code' }}</button>
    </div>
  </section>
</template>
